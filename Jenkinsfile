...
pipeline {
    ...
    stages {
        ...
        stage('deploy') {
            steps {
                /* groovylint-disable-next-line LineLength */
                sh "sudo ssh -o StrictHostKeyChecking=no -i $DEPLOYKEY ec2-user@$PRODEC2IP '[ -d '/home/ec2-user/app' ] && (cd ~/app;sudo docker-compose stop);echo hi'"
                /* groovylint-disable-next-line LineLength */
                ...
                sh "sudo scp -o StrictHostKeyChecking=no -i $DEPLOYKEY -p ./badreads-backend/.env ec2-user@$PRODEC2IP:~/app/badreads-backend"
                 /* groovylint-disable-next-line LineLength */
                sh "sudo scp -o StrictHostKeyChecking=no -i $DEPLOYKEY -p ./badreads-frontend/.env ec2-user@$PRODEC2IP:~/app/badreads-frontend"
                sh "sudo scp -o StrictHostKeyChecking=no -i $DEPLOYKEY -p ./init-mongo.js ec2-user@$PRODEC2IP:~/app"
                /* groovylint-disable-next-line LineLength */
                sh "sudo ssh -o StrictHostKeyChecking=no -i $DEPLOYKEY ec2-user@$PRODEC2IP 'cd ~/app;sudo docker-compose up --build --force-recreate -d;exit'"
            }
        }
    }
}
